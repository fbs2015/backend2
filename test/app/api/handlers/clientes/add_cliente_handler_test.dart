import 'package:backend2/app/api/api.dart';
...
void main() {
  ...
  setUpAll((){
    ...
    registerFallbackValue(Cliente(id: 1, nome: 'nome', email: 'email', telefone: 'telefone'));  
  });
 ...
 group('AddClienteHandler', () {
  ...
  test('deve retornar um ClienteOutputDTO', () async {
   ...
   test('deve retornar um status InternalServerError quando for erro generico ', () async {
  ...
  test('deve retornar um status badRequest quando existir um cliente com msm email ', () async {
    when(() => mockAddClienteUseCase.call(any())).thenThrow(JaExisteUmClienteComEsseEmailException());
    final result = await handler.call(RequestParams(
      body:{
        'nome': 'nome test',
        'email': 'email@email.com',
  ...
  test('deve retornar uma mensagem "Ja existe um cliente com esse email"', () async {
    when(() => mockAddClienteUseCase.call(any())).thenThrow(JaExisteUmClienteComEsseEmailException());
    final result = await handler.call(RequestParams(
      body:{
        'nome': 'nome test',
    ...
    expect((result.body as MessageError).message, "Ja existe um cliente com esse email");
  });
 });
}